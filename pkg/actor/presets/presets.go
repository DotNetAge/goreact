...
func NewResilientActor(tm *tools.Manager, opts ...Option) *ResilientActor {
	cfg := defaultConfig()
...
func NewDebugActor(tm *tools.Manager, tracer *debug.ExecutionTracer, profiler *debug.PerformanceProfiler) *DebugActor {
	return &DebugActor{
		toolManager: tm,
		tracer:      tracer,
		profiler:    profiler,
...
func NewSafeActor(tm *tools.Manager, opts ...Option) *SafeActor {
	cfg := defaultConfig()
	for _, opt := range opts {
		opt(cfg)
	}
...
func NewProductionActor(tm *tools.Manager, opts ...Option) *ProductionActor {
