...
在编排模式（如 Master-Sub）驱动的架构中，Actor 是整个系统中最危险、也是最接近物理机器的组件。由于删除、修改等高危指令可能并非出自顶层 Agent 的直接意图，而是来自于某个被召回的 `Skill` (Markdown 文本) 或 `CompiledAction` (肌肉记忆)，因此试图在 Thinker 层面做语义拦截既复杂又不可靠。**Actor 必须作为“原子级”的最后一道防线。**
