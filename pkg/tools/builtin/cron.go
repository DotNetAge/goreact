...
func (c *Cron) Description() string {
	return "Cron expression parsing and calculation. Params: {operation: 'parse'|'next'|'validate', expression: 'cron_expr', from: 'time_string', count: number}"
}

// SecurityLevel returns the tool's security risk level
func (t *Cron) SecurityLevel() tools.SecurityLevel {
