...
type SecurityPolicy func(toolName string, level core.SecurityLevel) bool
...
func (r *ToolRegistry) SetResultStorage(storage core.ToolResultStorage) {
	r.mu.Lock()
	defer r.mu.Unlock()
...
func (r *ToolRegistry) SetResultLimits(limits core.ToolResultLimits) {
	r.mu.Lock()
	defer r.mu.Unlock()
...
func (r *ToolRegistry) ResetMessageCharCounter() {
	r.mu.Lock()
	defer r.mu.Unlock()
...
func (r *ToolRegistry) SetSecurityPolicy(policy SecurityPolicy) {
	r.mu.Lock()
	defer r.mu.Unlock()
...
func (r *ToolRegistry) SetPermissionChecker(checker core.ToolPermissionChecker) {
	r.mu.Lock()
	defer r.mu.Unlock()
...
func (r *ToolRegistry) AddHook(hook core.Hook) {
	r.mu.Lock()
	defer r.mu.Unlock()
...
func (r *ToolRegistry) SetEventEmitter(fn func(core.ReactEvent)) {
	r.mu.Lock()
	defer r.mu.Unlock()
	r.eventEmitter = fn
}
